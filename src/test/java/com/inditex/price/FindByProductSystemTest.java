...
public class FindByProductSystemTest {
  ...
  @Test
  void findByProduct() {
    ResponseEntity<FindByProductResponseDTO> response = whenFindByProduct(
        getFindByProductRequestDTO());
    then(response.getStatusCode())
        .isEqualTo(HttpStatus.OK);

  }

  private ResponseEntity<FindByProductResponseDTO> whenFindByProduct(
      FindByProductRequestDTO findByProductRequestDTO) {
    HttpHeaders headers = new HttpHeaders();
    headers.add("Content-Type", "application/json");
    HttpEntity<FindByProductRequestDTO> request = new HttpEntity<>(findByProductRequestDTO,
        headers);
